...
#Ensure that below is set in the ve before you call
    ...
    for my_bucket_object in my_bucket.objects.filter(Prefix=link.partition('/')[2]):
        ...
        with rio.open('s3://{0}/{1}'.format(my_bucket.name, my_bucket_object.key)) as src:
            ...
            with rio.open('out.tiff') as origin:
                ...
                #old 
                with rio.open("dem.masked_dem" + ".tif", "w", **clipped_meta) as dest:
                    dest.write(clipped_raster) 
                    
                    
                localname='dem.masked_dem.tif'
                ...
                with rasterio.open(localname) as nf:
                    ...
                    with rio.open("dem.masked_dem2" + ".tif", "w", **clipped_meta) as dest:
                        dest.write(clipped_raster) 
            ...
            with rio.open('veg.tiff') as origin:
                ...
                #read in the clipped-resampled veg raster    
                localname='dem.masked2.tif'    
                with rasterio.open(localname) as nf:
                    print(nf.profile)
                    print(nf.crs)
                    print(nf.meta)
                    veg_r = nf.read(1) # read the entire array
                    
                    # open the dem raster
                    localrastername='dem.masked_dem2.tif'
                    with rasterio.open(localrastername) as dfr:
                        dem_r = dfr.read(1) # read the entire array
                    ...
                    #convert to float64
                    veg_64 = np.array(veg_r, dtype=numpy.float64)
                    dem_64 = np.array(dem_r, dtype=numpy.float64)
                    ndvi_64 = np.array(planet_ndvi , dtype=numpy.float64)
                    
                    # new path
                    new_bucket = s3.Bucket('planet-snowcover-imagery-veg-dem')
                    ...
                    with tempfile.NamedTemporaryFile() as tmpfile:
                        with rasterio.open(tmpfile.name,
                            ...
                            dstr.write_band(5, veg_64)
                            dstr.write_band(6, dem_64)
                        dstr.close()   
