        ...
        with rio.open('s3://{0}/{1}'.format(my_bucket.name, my_bucket_object.key)) as src:
            ...
            with rio.open('out.tiff') as origin:
                ...
                with rasterio.open(localname) as nf:
                    ...
                    # slope calculation
                    clippedarea_dem=rd.LoadGDAL("dem.masked2.tif")
                    slope = rd.TerrainAttribute(clippedarea_dem, attrib='slope_riserun')
                    aspect = rd.TerrainAttribute(clippedarea_dem, attrib='aspect')
                    ...
                    #slope = rd.TerrainAttribute(dem_64, attrib='slope_riserun')
                    #aspect = rd.TerrainAttribute(dem_64, attrib='aspect')
                    
                    ndvi_64 = np.array(planet_ndvi , dtype=numpy.float64)
                    slope_64 = np.array(slope , dtype=numpy.float64)
                    aspect_64 = np.array(aspect , dtype=numpy.float64)
                    new_bucket = s3.Bucket('planet-snowcover-dem-etc')
                    ...
                    with tempfile.NamedTemporaryFile() as tmpfile:
                        with rasterio.open(tmpfile.name,
                            ...
                            dstr.write_band(6, slope_64)
                            dstr.write_band(7, aspect_64)
